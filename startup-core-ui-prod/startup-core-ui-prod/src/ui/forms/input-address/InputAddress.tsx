...
const Maps = ({
  ...
  return (
    <div className="bg-white border rounded-lg mt-[-23px] p-3 relative translate-y-[calc(-100%+-38px)]">
      <div className="relative pt-[100%] md:pt-[66%]">
    ...
    <Popover>
      <div ref={myRef} className="relative w-full">
        <InputShell
          ...
          note={error ?? note}
          optional={
            withPSGC ? <InputGeolocToPSGC geolocName={geolocName} /> : undefined
          }
          error={error}
          trailing={
            <>
              {show ? (
                <div className="pt-0.5 translate-x-1.5">
        ...
        >
          <Popover.Panel
            static
            className="absolute z-10 inset-x-0 transform shadow-lg h-0"
          >
            <Maps
              setRef={handleSetRef}
