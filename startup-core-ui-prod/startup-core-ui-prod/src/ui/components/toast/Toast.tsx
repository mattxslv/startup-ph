...
const MAP_ICON = {
  default: (
    <HiCheckCircle className="h-6 w-6 text-description" aria-hidden="true" />
  ),
  success: (
    <HiCheckCircle className="h-6 w-6 text-success-base" aria-hidden="true" />
  ),
  error: (
    <HiExclamation className="h-6 w-6 text-danger-base" aria-hidden="true" />
  ),
  warning: (
    <HiExclamationCircle
      className="h-6 w-6 text-warning-base"
      aria-hidden="true"
    />
  ),
  info: (
...
function reducer(state: IToastState, action: any) {
  switch (action.type) {
    ...
    case 'OMIT_TOAST':
      return {
        ...state,
        items: state.items.filter((x: IToast) => x.id !== action.id),
      };
    default:
      // do nothing
      return state;
...
const ToastItem = (props: IToast) => {
  ...
  return (
    ...
    >
      <div
        className={clsx(
          'flex-shrink-0 pointer-events-auto w-full max-w-md overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 relative',
          MAP_BG_CLASS[props.type]
        )}
      >
        <div className="p-4">
          <div className="flex items-start">
            <div className="flex-shrink-0">{MAP_ICON[props.type]}</div>
            <div className="ml-3 w-0 flex-1 pt-0.5">
              <p className="text-base leading-[18px] font-semibold text-black">
                {props.message}
              </p>
              {props?.config?.description ? (
                <p className="mt-3 text-sm leading-5 text-description">
                  {props?.config?.description}
                </p>
              ) : null}
            </div>
            <div className="ml-4 flex flex-shrink-0">
              <button
                type="button"
                className={clsx(
                  'inline-flex rounded-md text-description hover:text-black focus:outline-none focus:ring-2 focus:ring-outline-active focus:ring-offset-2',
                  MAP_BG_CLASS[props.type]
                )}
                onClick={() => {
                  setShow(false);
        ...
        <div
          key={props.id}
          className={clsx(
            'absolute h-1 bottom-0 left-0 transition-all ease-linear',
            TIMEOUT_BAR_CLASS[props.type]
          )}
          style={{
            width: timeoutWidth,
