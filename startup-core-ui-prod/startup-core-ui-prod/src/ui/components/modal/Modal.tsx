...
const MAP_SIZE_CLASS = {
  ...
  xl2: 'w-full max-w-[1260px]',
  full: 'w-full max-w-[1560px]',
};
  ...
  return (
    <Transition.Root appear show={open} as={Fragment}>
      <Dialog
        ...
        onClose={closeOutsideClick ? setOpen : doNothing}
      >
        <Transition.Child
          as={Fragment}
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <Dialog.Panel
                className={clsx(
                  'relative transform rounded-lg border text-left bg-white shadow-modal transition-all w-full', // overflow-hidden
                  MAP_SIZE_CLASS[size ?? 'base']
                )}
              >
                {typeof title === 'string' ? (
                  <div className="p-6 border-b bg-fill-light flex justify-between">
                    <div className="font-semibold leading-[18px] truncate">
                      {title}
                    </div>
                    {titleClose ? (
                      <button
