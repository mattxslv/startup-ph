...
export const DropdownItem = ({
  ...
  return (
    ...
    >
      {leadingIcon ? (
        <span className='group-active:text-primary mr-3 text-description'>{leadingIcon}</span>
      ) : null}
      {children ? (
        <span className='group-active:text-primary group-disabled:text-disabled'>{children}</span>
      ) : null}
      {trailingIcon ? <span className='ml-auto text-description'>{trailingIcon}</span> : null}
    </button>
  );
};
    ...
    <DropdownContext.Provider value={{ show, setShow }}>
      ...
      >
        <div ref={divRef} className='relative'>
          <div className={clsx('absolute h-0', MAP_ALIGNMENT_CLASS[alignment!])} style={{ width }}>
