...
import { INews, fetchNewsById } from '@/feature/news/useNews';
...
function NewsItemPage({ data, pageNotFound }: Props) {
  const title = `Startup PH News`;
        ...
        <div className='bg-white p-6 rounded-none lg:rounded-lg relative'>
          ...
          <div>
            {Array.isArray(data?.body) ? (
              <ViewCampaign
                data={data?.body}
                message='Stay tuned! This exciting news article is on its way. Check back soon for the full story.'
              />
            ) : null}
          </div>
          {/* <hr /> */}
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const data = await fetchNewsById(context?.params?.id);
